...
const Dashboard = ({ filters, onFilterChange, theme }) => {
  ...
  const API_URL = "/api/data/"; // Relative URL for both local and production
  ...
  const fetchFilteredData = debounce((params) => {
    ...
    setError(null);
    axios
      .get(API_URL, { params }) // Use the API_URL constant
      .then((response) => {
        setFilteredData(response.data.results); // Use the 'results' array from the paginated response
        setLoading(false);
      })
      .catch((error) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div className="md:col-span-1">
        <Filters
          data={filteredData} // Pass filteredData to Filters
          filters={filters}
